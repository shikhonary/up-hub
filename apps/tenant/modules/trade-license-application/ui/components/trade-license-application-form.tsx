...
import { OrgInfoStep } from "./steps/org-info-step";
...
import { useCreateTradeLicenseApplication, useUpdateTradeLicenseApplication } from "@workspace/api-client";
...
const steps = [
  { id: 1, title: "প্রতিষ্ঠানের তথ্য", sub: "প্রাথমিক তথ্য" },
  { id: 2, title: "মালিকের তথ্য", sub: "পরিচয় ও পারিবারিক" },
  { id: 3, title: "ঠিকানা", sub: "বর্তমান ও স্থায়ী" },
  { id: 4, title: "ব্যবসার বিস্তারিত", sub: "ক্যাটাগরি ও যোগাযোগ" },
...
export const TradeLicenseApplicationForm = ({
  id,
  initialData,
}: TradeLicenseApplicationFormProps) => {
  const [currentStep, setCurrentStep] = useState(1);
  const [isEnglishEnabled, setIsEnglishEnabled] = useState(false);
  const router = useRouter();

  const createMutation = useCreateTradeLicenseApplication();
  const updateMutation = useUpdateTradeLicenseApplication();

  const isPending = createMutation.isPending || updateMutation.isPending;

  const form = useForm<TradeLicenseApplicationFormValues>({
    resolver: zodResolver(tradeLicenseApplicationSchema) as any,
    mode: "onChange",
    defaultValues: initialData || {
      ...
      permanentVillageBn: "",
      permanentVillageEn: "",
      permanentHoldingNoBn: "",
      permanentHoldingNoEn: "",
      permanentWardNo: 0,
  ...
  const nextStep = async (e?: React.MouseEvent) => {
    ...
    if (currentStep === 1) {
      fieldsToValidate = ["orgNameBn", "ownershipTypeBn", "fiscalYearId"];
      if (isEnglishEnabled) fieldsToValidate.push("orgNameEn");
  ...
  const onSubmit = async (values: TradeLicenseApplicationFormValues) => {
    ...
    try {
      ...
      router.push("/trade-license/applications");
    } catch (error: any) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-20">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 bg-white p-8 md:p-10 rounded-[32px] shadow-[0_2px_20px_-8px_rgba(0,0,0,0.05)] border border-slate-100/50 relative overflow-hidden">
        {/* Decorative background element */}
        <div className="absolute -top-24 -right-24 w-64 h-64 bg-primary/5 rounded-full blur-3xl" />

        <div className="flex items-start gap-6 relative z-10">
          <div className="w-16 h-16 rounded-[20px] bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center text-white shadow-[0_8px_16px_-6px_rgba(var(--primary),0.4)] shrink-0">
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="px-3 py-1 bg-emerald-50 text-emerald-600 text-[11px] font-black uppercase tracking-wider rounded-full border border-emerald-100/50">
                নতুন আবেদন
              </span>
            </div>
            <p className="text-sm font-medium text-slate-500 max-w-xl leading-relaxed">
              ইউনিয়ন পরিষদের ডিজিটাল সেবার মাধ্যমে আপনার ব্যবসার ট্রেড লাইসেন্স
              আবেদন করুন। ধাপে ধাপে সঠিক তথ্য দিয়ে আবেদন প্রক্রিয়া সম্পন্ন করুন।
            </p>
          </div>
        </div>

        <div className="flex items-center gap-4 bg-slate-50/50 p-4 rounded-[24px] border border-slate-100 relative z-10 shrink-0">
      ...
      <Card className="border-none shadow-ambient rounded-[40px] overflow-hidden bg-white/70 backdrop-blur-xl border border-white/20">
